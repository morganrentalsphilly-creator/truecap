...
function classifyCapRate(rate: number): { label: string; color: string; desc: string } {
  if (rate < 0) return { label: "Negative", color: "text-[var(--metric-negative)]", desc: "Operating at a loss." };
  if (rate < 4) return { label: "Below market", color: "text-[var(--metric-negative)]", desc: "Lower than most US rental markets." };
...
export function CapRateCalculatorWidget() {
  ...
  const { capRate, noi, opEx, annualRent } = useMemo(() => {
    ...
    const opexPct = Math.max(0, Math.min(100, numberOrZero(opexPctInput))) / 100;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="bg-[var(--background)] rounded-xl border border-border p-5 sm:p-6 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">{classification.desc}</p>
          </div>
          ...
          <div className="mt-5 pt-5 border-t border-border space-y-1.5 text-xs">
            ...
            <Row label="Net Operating Income (NOI)" value={fmtMoney(noi)} bold />
            <Row label="Purchase price" value={fmtMoney(numberOrZero(priceInput))} />
          </div>
        </div>
      </div>
