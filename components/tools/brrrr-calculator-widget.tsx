  ...
  return (
    <div className="bg-card rounded-2xl border border-border shadow-sm p-5 sm:p-7">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
        ...
        <Pct
          label="Closing costs (acq)"
          value={closeAcq}
          setValue={setCloseAcq}
        />
        ...
        <Pct
          label="Refi rate"
          value={refiRate}
          setValue={setRefiRate}
          step="0.125"
        />
        <Plain
          label="Refi term (yrs)"
          value={refiTerm}
          setValue={setRefiTerm}
        />
        <Pct label="Refi closing %" value={closeRefi} setValue={setCloseRefi} />
        <Money
          label="Monthly carry (rehab)"
          value={carry}
          setValue={setCarry}
        />
      ...
      <div className="rounded-xl border border-border bg-[var(--background)] p-5 sm:p-6 space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          <Metric
            ...
            value={fmt(result.cashLeftInDeal)}
            positive={result.cashLeftInDeal === 0}
            negative={
              result.cashLeftInDeal > 0 &&
              result.cashLeftInDeal >= result.totalCashInvested
            }
          />
          <Metric
            label="Cash returned at refi"
            value={fmt(result.cashReturnedAtRefi)}
            positive={result.cashReturnedAtRefi > 0}
          />
          <Metric
            label="Post-refi CF"
            value={`${fmt(result.postRefiMonthlyCashFlow)}/mo`}
          ...
          <Metric
            label="Post-refi CoC"
            value={
              result.isInfiniteReturn
                ? "∞ Infinite"
                : fmtPct(result.postRefiCashOnCashPct)
            }
            positive={
              result.isInfiniteReturn || result.postRefiCashOnCashPct > 10
            }
            negative={
              !result.isInfiniteReturn && result.postRefiCashOnCashPct < 0
            }
          />
        </div>
        ...
        {result.cashNeededAtRefi > 0 && (
          <p className="text-xs font-semibold text-[var(--metric-negative)]">
            ...
            {fmt(result.cashNeededAtRefi)} to the refi table (included in cash
            left in deal).
          </p>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div>
            ...
            <Row
              label="Closing costs"
              value={fmt(result.originalClosingCosts)}
            />
            <Row label="Rehab budget" value={fmt(result.rehabBudget)} />
            <Row
              label="Carrying costs"
              value={fmt(result.carryingCostsTotal)}
            />
            <Row
              label="Total cash invested"
              value={fmt(result.totalCashInvested)}
              bold
            />
          </div>
          <div>
            ...
            <Row
              ...
              value={fmt(result.refiClosingCosts)}
            />
            {result.cashNeededAtRefi > 0 ? (
              <Row
                label="Cash needed at refi"
                value={fmt(result.cashNeededAtRefi)}
                bold
              />
            ) : (
              <Row
            ...
            <Row
              ...
              value={fmt(result.newMonthlyPayment)}
            />
            <Row
              label="Equity created"
              value={fmt(result.equityCreated)}
              bold
            />
          </div>
        </div>
      ...
      >
        ...
        Run the full analysis with these numbers — save BRRRRs, compare them,
        export PDFs — free in TrueCap
