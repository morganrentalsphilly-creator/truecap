...
function classify(coc: number): { label: string; color: string; note: string } {
  if (coc < 0)
    return {
      ...
      note: "Money out of pocket each year.",
    };
  if (coc < 4)
    return {
      label: "Below market",
      color: "text-[var(--metric-negative)]",
      note: "Likely under-leveraged or over-priced.",
    };
  if (coc < 8)
    return {
      label: "Modest",
      color: "text-foreground",
      note: "Common for appreciation-focused markets.",
    };
  if (coc < 12)
    return {
      label: "Healthy",
      color: "text-[var(--metric-positive)]",
      note: "Typical target for buy-and-hold investors.",
    };
  if (coc < 20)
    return {
      label: "Strong",
      color: "text-[var(--metric-positive)]",
      note: "Cash-flow-heavy deal.",
    };
  return {
    label: "Exceptional — verify",
    color: "text-[var(--metric-positive)]",
    note: "Sanity-check rent, expenses, and price.",
  };
}
    ...
    <div className="bg-card rounded-2xl border border-border shadow-sm p-5 sm:p-7">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        ...
        <div className="space-y-4">
          ...
          <FieldPct
            label="Operating Expenses (% of rent)"
            value={opexPct}
            setValue={setOpexPct}
          />
        </div>
        ...
        <div className="bg-[var(--background)] rounded-xl border border-border p-5 sm:p-6 flex flex-col justify-between">
          ...
          <div className="mt-5 pt-5 border-t border-border space-y-1.5 text-xs">
            <Row
              ...
              value={fmtMoney(result.annualCF)}
              bold
            />
            ...
            <Row
              ...
              value={fmtMoney(result.opex)}
            />
            <Row
              label="Cash invested (down + closing)"
              value={fmtMoney(result.cashIn)}
              bold
            />
          </div>
        </div>
