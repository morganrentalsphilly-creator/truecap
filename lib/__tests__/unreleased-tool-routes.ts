 ...
 *
 *   notFound()          - retained as a detector for any future fail-closed
 *                         route that has not yet received an equity-preserving
 *                         destination.
 *   permanentRedirect() - all ten historical calculator URLs now use this
 *                         while unreleased, including the feature-gated BRRRR
 *                         page and the always-retired property-tax page.
...
export function unusableToolRoutes(
  root = process.cwd(),
): Map<string, UnusableReason> {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    const code = source
      .replace(/\/\*[\s\S]*?\*\//g, " ")
      .replace(/^\s*\/\/.*$/gm, " ");
    if (/\b(permanentRedirect|redirect)\s*\(/.test(code)) {
...
export function targetsUnusableTool(
  href: string,
  root = process.cwd(),
): boolean {
