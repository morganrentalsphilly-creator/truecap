...
describe("global skip-link destinations", () => {
  ...
  it("keeps every admin email-preview return branch inside #main", () => {
    expect(read("app/admin/email-preview/page.tsx").match(/<main id="main"/g)).toHaveLength(4);
  });
});
...
describe("public calculator validation and claims", () => {
  ...
  it("keeps the quick tools factual instead of recommending an investment", () => {
    ...
    expect(source).not.toMatch(/better than most index funds|strong cash flow play|don.t deploy capital/i);
...
describe("dashboard accessibility and recovery", () => {
  ...
  it("names the workspace My Deals and gives route errors a retry", () => {
    expect(read("app/dashboard/saved-analyses/page.tsx")).toContain('title: "My Deals"');
    expect(read("components/investcalc/saved-analyses-page-v2.tsx")).toContain(">My Deals</h1>");
    expect(read("app/dashboard/page.tsx")).toContain("<RetryRouteButton");
    expect(read("app/dashboard/saved-analyses/page.tsx")).toContain("<RetryRouteButton");
    expect(read("app/dashboard/templates/page.tsx")).toContain("<RetryRouteButton");
    expect(read("app/dashboard/compare/page.tsx").match(/<RetryRouteButton/g)).toHaveLength(2);
    expect(read("components/dashboard/retry-route-button.tsx")).toContain("router.refresh()");
  });
  ...
  it("uses deal-count-aware mobile comparison columns", () => {
    ...
    expect(read("lib/compare-responsive.ts")).toContain('return "grid-cols-2 sm:grid-cols-4"');
  });
...
describe("non-pointer help and concise announcements", () => {
  it("opens quick-screen definitions by keyboard or tap", () => {
    ...
    expect(source).toContain('aria-label={`${c.label}: ${c.value}. Show definition`}');
  ...
  it("announces only the Offer Ceiling summary, not the whole result card", () => {
    ...
    expect(source).not.toMatch(/<div\s+aria-live="polite"\s+aria-atomic="true"/);
  });
...
describe("browser capability recovery and crawl path", () => {
  it("surfaces blocked tabs and storage, and avoids async PDF popups", () => {
    ...
    expect(deals).toContain("openPdfUrlInCurrentTab");
    expect(deals).toContain("window.location.assign(pdfUrl)");
  });
