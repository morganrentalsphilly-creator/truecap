...
describe("comparison neutrality guards", () => {
  ...
  it("frames relative metric positions as modeled comparisons, not investment directives", () => {
    ...
    expect(comparison).toContain(
      "does not establish safety or make an investment recommendation",
    );
    expect(comparison).toContain(
      "subtle row shading marks the highest or lowest displayed value",
    );
    ...
    expect(comparison).not.toMatch(
      /Most metric wins|Relative leader|Metric leader|Strongest DSCR|Comparison reference/,
    );
    expect(comparison).not.toMatch(
      /Near-term score|Long-term score|lead count/,
    );
  ...
  it("offers scale-aware review and makes assumption differences inspectable", () => {
    ...
    expect(comparison).toContain(
      "scaling dollar values does not align the underlying assumptions",
    );
  ...
  it("gates unreleased tax and exit surfaces and records one privacy-safe evaluation use", () => {
    ...
    expect(ANALYTICS_EVENT_DICTIONARY.evaluation_comparison_used).toEqual({
      owner: "product",
      privacy: "account-aggregate",
      allowedProperties: ["count_bucket"],
    });
  });
  ...
  it("describes the chart axes without calling model DSCR a safety verdict or target", () => {
    ...
    expect(chart).toContain(
      "Neither direction establishes safety or recommends a deal",
    );
    expect(chart).toContain("Above both references");
    expect(chart).toContain("Below both references");
    expect(chart).toContain(
      "fixed comparison references, not your adopted targets",
    );
    expect(metrics).toContain('label: "Model DSCR"');

    expect(chart).not.toMatch(
      /safe \+ strong return|Target ✓|Higher risk|DSCR \(safer/,
    );
  });
