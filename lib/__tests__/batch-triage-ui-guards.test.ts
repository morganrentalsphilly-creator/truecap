...
describe("batch triage review accessibility guards", () => {
  it("labels editable preview fields and connects inline validation", () => {
    const editor = source.slice(
      source.indexOf("function PreviewInput"),
      source.indexOf("function PreviewRowContext"),
    );
    expect(editor).toMatch(/<label\s+[^>]*htmlFor=\{inputId\}/);
  ...
  it("keeps primary, filtering, and sorting controls at least 44px high", () => {
    expect(source).toMatch(
      /Review listings[\s\S]*?min-h-11|className="inline-flex min-h-11[^"]*"[\s\S]*?Review listings/,
    );
  ...
  it("uses cards through tablet widths and tables only at desktop", () => {
    ...
    expect(source).toContain(
      'className="mt-4 hidden overflow-hidden rounded-2xl border border-border lg:block"',
    );
  });
});
