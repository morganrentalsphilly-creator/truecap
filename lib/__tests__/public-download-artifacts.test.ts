...
describe("public downloadable artifacts", () => {
  it("renders all-cash DSCR as the exact not-applicable label", () => {
    ...
    expect(xml).toContain(
      'IF(B25&gt;0,(B11-B40)/B25,"N/A — no debt service")'
    );
    expect(xml).toContain(
  ...
  it("describes both downloads within their released scope", () => {
    ...
    expect(capture).toContain("buy-and-hold screening references");
    expect(delivery).toContain("not strategy recommendations");
  });
});
