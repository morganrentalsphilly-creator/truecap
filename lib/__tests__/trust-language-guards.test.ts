...
describe("trust-language guards", () => {
  ...
  it("keeps the PDF versioned and explicit about benchmark limitations", () => {
    ...
    expect(generator).toContain(
      "highest modeled profit is not a recommendation",
    );
  ...
  it("keeps refinance, appraisal, LTV, credit, and DTI guidance program-specific", () => {
    ...
    expect(combined).not.toMatch(
      /squeaking under the wire with automated approval/i,
    );
    expect(combined).not.toMatch(/the refi is still doable/i);
    expect(combined).not.toMatch(
      /delayed-financing exception lets .* immediately/i,
    );
  ...
  it("does not promise a fixed state-tax lift in market content", () => {
    ...
    expect(combined).not.toMatch(
      /Arizona often beats Texas on net carrying cost/i,
    );
  ...
  it("does not publish unsupported appraisal, reserve, insurance, or source-authority shortcuts", () => {
    ...
    expect(cityStrategies).not.toMatch(
      /Cleveland appraisals (?:run|consistently come in)/i,
    );
  ...
  it("keeps market, voucher, operator, appraisal, and lender outcomes verification-first", () => {
    const cityStrategies = read("../city-strategy-combos.ts");
    const cityStrategyPage = read(
      "../../app/markets/[city]/[strategy]/page.tsx",
    );
    const proForma = read(
      "../../app/blog/rental-property-pro-forma-explained/page.tsx",
    );
    const voucher = read(
    ...
    expect(`${voucher}\n${philadelphia}`).not.toMatch(
      /guaranteed rent \+ automatic renewals/i,
    );
    ...
    expect(houston).not.toMatch(
      /Texas evictions move fast \(21-45 days uncontested\)/i,
    );
    expect(atlanta).not.toMatch(
      /Georgia evictions move faster[\s\S]{0,80}30-45 days/i,
    );
    expect(memphis).not.toMatch(/insurance runs \$1,200-2,400\/yr/i);
    expect(memphis).not.toMatch(/your net is typically 2-4% lower/i);

    expect(lenderCopy).not.toMatch(
      /Most DSCR lenders won(?:'|&apos;)t fund anything below 1\.0/i,
    );
    expect(lenderCopy).not.toMatch(
      /Trying to use DSCR for the acquisition usually fails/i,
    );
  ...
  it("keeps the retired refund guarantee fail-closed", () => {
    ...
    expect(guaranteePage).toContain(
      "No public TrueCap refund guarantee is currently offered.",
    );
