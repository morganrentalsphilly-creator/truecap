 ...
 * we hand to the analyzer's existing address enrichment (Google Places plus
 * HUD/FRED benchmarks; property tax stays manual). Always surfaced as "parsed from the link — confirm it",
...
export function parseListingUrl(raw: string): ParsedListing | null {
  ...
  if (host.includes("zillow.") || host.includes("trulia.")) {
    ...
    if (slug) {
      const out = finalize(
        slug,
        host.includes("trulia.") ? "trulia" : "zillow",
      );
      if (out) return out;
    }
  }
  ...
  if (host.includes("redfin.")) {
    ...
    if (hi >= 2) {
      ...
      const address = `${deslug(addrSlug)} ${deslug(city)} ${st}`
        .replace(/\s+/g, " ")
        .trim();
      if (/\d/.test(address)) {
        return {
          address,
          state: US_STATES.has(st.toUpperCase())
            ? st.toUpperCase()
            : findState(address),
          zip: findZip(address),
          source: "redfin",
