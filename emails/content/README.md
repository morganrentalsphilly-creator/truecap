...
be the **Monday** of the week (ISO date format: `YYYY-MM-DD.json`).

## How it works

The cron at `/api/cron/send-weekly-digest` fires every Monday at 9am ET.
It looks for a file whose date matches the current Monday. If found,
it renders the email and sends to the audience. If no file exists for
this Monday, it silently skips — so missing a week is harmless.

## Filename rule

`YYYY-MM-DD.json` where the date is the **Monday** the email should go out.

Examples:
- `2026-06-01.json` — sends Monday June 1, 2026 at 9am ET
- `2026-06-08.json` — sends Monday June 8, 2026 at 9am ET

## Required fields

See `2026-06-01.json` for a full annotated example. Required:
...
1. Copy the most recent file → rename to next Monday's date
...
7. The cron sends automatically Monday at 9am ET
