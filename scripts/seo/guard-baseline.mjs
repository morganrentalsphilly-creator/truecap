 ...
 *   node scripts/seo/guard-baseline.mjs --write --adopt slug-a,slug-b
 *
 ...
 *
 * --adopt is the ONE sanctioned escape hatch, and it is deliberately awkward:
 * you must name each slug. It exists for posts that were authored BEFORE this
 * guard existed but landed on main after it — genuinely pre-existing debt that
 * the "new posts meet the standard outright" rule would otherwise treat as a
 * regression forever, leaving CI permanently red.
 *
 * It is NOT for making a failing new post pass. If a post the content
 * workflow just wrote trips the guard, the post is what's wrong — the whole
 * point of the gate is that automated content cannot lower the bar by
 * arriving. Every adopted slug is recorded in `adopted` in the JSON with the
 * date, so the shortcut is visible in review rather than silent.
 *
...
/**
 * Slugs named via `--adopt a,b,c` — posts authored before this guard existed
 * that are being recorded as pre-existing debt rather than treated as new
 * content failing the bar. See the header comment; this is not a way to pass
 * a freshly-written post.
 */
const adoptArg = process.argv.find((a) => a.startsWith("--adopt"));
const ADOPT = new Set(
  (adoptArg?.includes("=")
    ? adoptArg.split("=")[1]
    : process.argv[process.argv.indexOf("--adopt") + 1] ?? ""
  )
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean),
        ...
        if (prof[family] < target) {
          problems.push(
            `new post ${slug} ships below standard: ${family} ${prof[family]}/${target}` +
              ` — fix the post, or if it predates this guard: --adopt ${slug}`,
          );
        }
      }
      continue;
...
if (previous) {
  if (adopted.length) {
    console.log(`\nAdopted ${adopted.length} pre-guard post(s) as recorded debt:`);
  ...
  `${JSON.stringify(
    {
      ...
      adopted: [
        ...
        ...adopted.map((slug) => ({
          slug,
          adoptedOn: new Date().toISOString().slice(0, 10),
          note: "Authored before lib/__tests__/seo-guards.test.ts existed; recorded as pre-existing debt rather than held to the new-post standard retroactively.",
        })),
      ],
      limits: {
