...
type ResultSnapshot = {
  ...
  /** Optional recorded stress result. Compare never generates one on read; it
   * only displays these fields when the saved snapshot already contains them. */
  downsideScenario?: unknown;
};
...
function mapDeal(
  ...
  const recordedDownside = asRecord(snapshot.downsideScenario);
  const metrics = {
    netCashFlow,
    cocReturn,
    capRate,
    ...
    // A downside result is not part of every historical save. Never generate
    // one silently in Compare: show it only when the recorded snapshot carries
    // the exact values, otherwise the optional rows stay absent.
    downsideNetCashFlow: toNumber(
      recordedDownside?.monthlyCashFlow as number | string | null | undefined
    ),
    downsideDscr: toNumber(
      recordedDownside?.dscr as number | string | null | undefined
    ),
  };

  const assumptions = buildDealAssumptions(row.form_snapshot, row, snapshot);
...
async function loadComparePickerDeals(
  supabase: Awaited<ReturnType<typeof createServerSupabaseClient>>,
  userId: string
): Promise<{ deals: ComparePickerDeal[]; hasError: boolean }> {
  const pickerSelect =
    "id, address, title, net_cash_flow_monthly, methodology_version, result_snapshot, form_snapshot";
  ...
  const deals = ((pickerRows ?? []) as unknown[])
    ...
    const scenario =
      typeof r.scenario_name === "string" && r.scenario_name.trim()
    ...
    return {
      ...
      signal:
        score != null && recommendation
          ? recommendationToSignal(recommendation)
          : null,
  ...
  return { deals, hasError: false };
}

function ComparePickerScreen({
  pickerDeals,
  pickerError,
  initialSelectedIds,
  displayName,
          ...
          <div className="mb-5 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {pickerDeals.length >= 2
                ...
                : "You need at least 2 active saved deals to compare."}
            </p>
          </div>
          {staleSelectionRecovered ? (
            <p role="status" className="mb-4 rounded-2xl border border-warning/40 bg-warning/10 px-4 py-3 text-sm text-warning-foreground">
              A previously selected deal is no longer active. The remaining deal is still selected; choose at least one more to continue.
            </p>
          ) : null}
          {pickerError ? (
            <div role="alert" className="rounded-2xl border border-destructive/30 bg-destructive/5 p-6 text-center shadow-sm">
              <p className="font-semibold text-foreground">Couldn&apos;t load your saved deals</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Your deals are still saved. Retry the comparison when the connection recovers.
              </p>
              <RetryRouteButton className="mt-4 rounded-full" />
            </div>
          ) : pickerDeals.length >= 2 ? (
            <CompareDealPicker deals={pickerDeals} initialSelectedIds={initialSelectedIds} />
          ) : (
...
export default async function DashboardComparePage() {
  ...
  if (ids.length < 2) {
    // A one-ID cookie is only a seed for the picker. Comparison itself always
    // requires at least two current active deals.
    const { deals: pickerDeals, hasError: pickerError } =
      await loadComparePickerDeals(supabase, user.id);

    return (
      <ComparePickerScreen
        pickerDeals={pickerDeals}
  ...
  // Stale-cookie recovery: if filtering deleted/archived/completed/invalid
  // snapshots leaves fewer than two active deals, never render a one-column
  // comparison. Reopen the in-place picker with any valid survivor seeded.
  if (deals.length < 2) {
    return (
      <ComparePickerScreen
        pickerDeals={pickerLoad.deals}
        pickerError={pickerLoad.hasError}
